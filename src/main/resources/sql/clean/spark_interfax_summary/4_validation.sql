WITH sha_union_step AS (
    SELECT DISTINCT id_sha2 FROM {dbSchema}.union_spark_interfax_summary WHERE id_sha2 IS NOT NULL
),
     sha_collision_step (
                         SELECT id_sha2,
                         max(CASE
         ...
         FROM {dbSchema}.collisions_report_spark_interfax_summary
         WHERE id_sha2 IS NOT NULL
         GROUP BY id_sha2
         ), sha_clean_step (
     SELECT DISTINCT id_sha2 FROM {dbSchema}.clean_spark_interfax_summary WHERE id_sha2 IS NOT NULL
), sha_eventlog_step (
      SELECT DISTINCT id_sha2 FROM {dbSchema}.clean_spark_interfax_summary
      WHERE
