with sha_union_step as (
	SELECT distinct id_sha2 FROM {dbSchema}.union_credit_correction where id_sha2 is not null
), sha_collision_step (
    SELECT id_sha2,
      max(case
    ...
    FROM {dbSchema}.collisions_report_credit_correction
    where id_sha2 is not null
    group by id_sha2
), sha_clean_step (
     SELECT distinct id_sha2 FROM {dbSchema}.clean_credit_correction where id_sha2 is not null
), sha_eventlog_step (
      SELECT distinct id_sha2 FROM {dbSchema}.clean_credit_correction
      where
