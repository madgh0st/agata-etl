WITH sha_union_step AS (
    SELECT DISTINCT id_sha2 FROM {dbSchema}.union_fssp WHERE id_sha2 IS NOT NULL
),
     sha_collision_step AS (
         SELECT id_sha2,
                max(CASE WHEN error_status != 'OK' THEN 'md_mapping_reason' ELSE NULL END) AS reason_filtered
         FROM {dbSchema}.collisions_report_fssp
         WHERE id_sha2 IS NOT NULL
         GROUP BY id_sha2
     ),
     sha_clean_step AS (
         SELECT DISTINCT id_sha2
         FROM {dbSchema}.clean_fssp
         WHERE id_sha2 IS NOT NULL
     ),
     sha_eventlog_step AS (
         SELECT DISTINCT id_sha2
         FROM {dbSchema}.clean_fssp
         WHERE
             code IS NOT NULL
