WITH sha_union_step AS (
    SELECT DISTINCT id_sha2 FROM {dbSchema}.union_genproc WHERE id_sha2 IS NOT NULL
),
     sha_collision_step (
                         SELECT id_sha2,
                         max(CASE
     ...
     SELECT DISTINCT id_sha2 FROM {dbSchema}.clean_genproc WHERE id_sha2 IS NOT NULL
), sha_eventlog_step (
      SELECT DISTINCT id_sha2 FROM {dbSchema}.clean_genproc
      WHERE
